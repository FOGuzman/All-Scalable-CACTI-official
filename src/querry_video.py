...
# Full
Qh,Qw,Qf = (args.resolution[0],args.resolution[0],args.frames*args.spix**2)

coords = np.expand_dims(np.nonzero(np.ones((args.resolution[0],args.resolution[0],args.frames*args.spix**2),dtype=np.uint16)),axis=0)
...
save_mp4(ten_out,"./implicit_results/clip_5_ir.mp4")

implay(ten_out.cpu().numpy())# 0.00687



coords_x2 = torch.unsqueeze(torch.nonzero(torch.ones((args.resolution[0],args.resolution[0],200))),0).float()
coords_x2[:,:,0] = (coords_x2[:,:,0]/(args.resolution[0]-1)*2)-1
coords_x2[:,:,1] = (coords_x2[:,:,1]/(args.resolution[0]-1)*2)-1
coords_x2[:,:,2] = (coords_x2[:,:,2]/(args.frames*args.spix**2-1)*2)-1
...
ten_out = torch.reshape(out_x2[:,:,0],(args.resolution[0]*2,args.resolution[0]*2,200))
